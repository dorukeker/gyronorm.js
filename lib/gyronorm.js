...
(function(root, factory) {
  ...
  */
  var GyroNorm = function(options) {
    ...
    this._gravityNormalized   = isPresent(options.gravityNormalized) ? options.gravityNormalized : DEFAULTS.gravityNormalized;
    this._screenAdjusted      = isPresent(options.screenAdjusted)    ? options.screenAdjusted    : DEFAULTS.screenAdjusted;
  ...
  *
  */
  GyroNorm.prototype.snapShot = function() {
    var doSnapShot = {};

    if (this._screenAdjusted) {
      doSnapShot = this._getScreenAdjustedEuler();
    } else {
      doSnapShot = this._do.getFixedFrameEuler();
    }

    var accSnapShot = this._dm.getScreenAdjustedAcceleration();
    var accGraSnapShot = this._dm.getScreenAdjustedAccelerationIncludingGravity();
    var rotRateSnapShot = this._dm.getScreenAdjustedRotationRate();

    var alphaToSend = 0;

    if (this._orientationBase === GAME) {
      alphaToSend = doSnapShot.alpha - this._calibrationValue;
      alphaToSend = (alphaToSend < 0) ? (360 - Math.abs(alphaToSend)) : alphaToSend;
    } else {
      alphaToSend = doSnapShot.alpha;
    }

    var dc = this._decimalCount;
    ...
    // Normalize gravity
    if (this._gravityNormalized) {
      snapShot.dm.gx *= this._gravityCoefficient;
      snapShot.dm.gy *= this._gravityCoefficient;
      snapShot.dm.gz *= this._gravityCoefficient;
    }
  ...
  */
  ...
  GyroNorm.prototype._getScreenAdjustedAccelerationIncludingGravity = function() {
    ...
    return this._dm.getScreenAdjustedAccelerationIncludingGravity();
  };
