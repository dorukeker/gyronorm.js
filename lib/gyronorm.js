...
(function(root, factory) {
  ...
  */
  var GyroNorm = function(options) {
    ...
    this._gravityNormalized   = !isNullOrUndefined(options.gravityNormalized) ? options.gravityNormalized : DEFAULTS.gravityNormalized;
    this._screenAdjusted      = !isNullOrUndefined(options.screenAdjusted)    ? options.screenAdjusted    : DEFAULTS.screenAdjusted;
  ...
  */
  GyroNorm.prototype.init = function() {
    ...
    var deviceOrientationPromise = this._initializeDeviceOrientationController();

    var deviceMotionPromise = this._initializeDeviceMotionController();
  ...
  */
   ...
   */
  GyroNorm.prototype._initializeDeviceMotionController = function() {
    var _this = this;
    return new FULLTILT.getDeviceMotion().then(function(controller) {
      _this._dm = controller;
      // Set gravity coefficient
      _this._gravityCoefficient = (controller.getScreenAdjustedAccelerationIncludingGravity().z > 0) ? -1 : 1;
    });
  };
   ...
   */
  GyroNorm.prototype._initializeDeviceOrientationController = function() {
    var _this = this;
    return new FULLTILT.getDeviceOrientation({ 'type': this._orientationBase }).then(function(controller) {
